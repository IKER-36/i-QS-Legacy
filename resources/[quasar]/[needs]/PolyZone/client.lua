...
local function _pointInPoly(point, poly)
  ...
  --print(point, ESX.DumpTable(poly))
...
function PolyZone:isPointInside(point, table)
  if table == nil then
    ...
    return _pointInPoly(point, table)
  end
end
...
local tabella = {}
local thread = false
-- recreated the main function for a performance fix
function PolyZone:onPointInOut(getPointCb, onPointInOutCb, waitInMS)
  ...
  AddEventHandler('change:status'..number, function(inside, point, center)
    if self.center == center then 
      onPointInOutCb(inside, point)
    end
  end)
end
...
Citizen.CreateThread(function()
  while true do 
    ...
    for k, v in pairs(casa) do 
      local distance = #(vector2(point.x, point.y) - v.center)
      if distance < 500 then
       if not v.destroyed then
         if not v.paused then
           local newIsInside = v:isPointInside(point, v)

           if tabella[v.center] == nil then
             tabella[v.center] = false
           end

           if tabella[v.center] ~= newIsInside then
              TriggerEvent('change:status'..k, newIsInside, point, v.center)
              tabella[v.center] = newIsInside
           end
         end
       end
      end
